...
const CardProduct: FC<TypeProduct> = ({ image, price, title, rate, description, id }) => {
  return (
    <Link
      className='block border md:border-2 shadow-gray-custom border-gray-custom p-2 rounded-lg w-44 md:w-60 bg-white-custom hover:scale-105 transition-transform'
      href={`/product/${id}`}
    >
      <img
        src={image}
